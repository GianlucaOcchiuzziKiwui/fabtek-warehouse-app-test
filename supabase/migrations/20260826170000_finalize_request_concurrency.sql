...
add column snapshot_track_inventory boolean;

update public.material_request_lines line
set snapshot_track_inventory = exists (
  select 1
  from public.inventory_movements movement
  where movement.request_line_id = line.id
    and movement.movement_type = 'reservation'
    and movement.reserved_delta > 0
);

alter table public.material_request_lines
alter column snapshot_track_inventory set not null;

comment on column public.material_request_lines.snapshot_track_inventory is
  'Immutable stock-tracking mode captured when the request line is submitted. Historical rows are tracked only when a reservation movement proves that stock was reserved.';
...
begin
  ...
  select
    line.item_variant_id,
    line.snapshot_track_inventory,
    line.requested_quantity,
    line.fulfilled_quantity
  into v_item_variant_id, v_track_inventory, v_requested, v_fulfilled
  from public.material_request_lines line
  where line.id = p_request_line_id
    and line.request_id = v_request_id
  for update;
  ...
  if found then
    ...
    select line.status, request.status
