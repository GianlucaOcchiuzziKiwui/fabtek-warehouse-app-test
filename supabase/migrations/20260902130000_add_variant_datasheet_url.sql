...
alter table public.item_variants
add constraint item_variants_datasheet_url_http_check check (
  datasheet_url is null or datasheet_url ~* '^https?://[^[:space:]]+$'
);

drop function public.save_catalog_variant(
...
begin
  ...
  if p_id is null then
    insert into public.item_variants (
      component_id, fabtek_code, oracle_sapio_code, datasheet_url, description,
      diameter, material, connection, unit_of_measure_id, track_inventory,
      sort_order, is_active
    ) values (
      p_component_id, btrim(p_fabtek_code), nullif(btrim(p_oracle_sapio_code), ''),
      nullif(btrim(p_datasheet_url), ''), btrim(p_description),
      nullif(btrim(p_diameter), ''), btrim(p_material), btrim(p_connection),
      p_unit_of_measure_id, p_track_inventory, p_sort_order, p_is_active
    ) returning id into v_variant_id;
  else
    select iv.id into v_variant_id
  ...
  delete from public.item_variant_categories where item_variant_id = v_variant_id;
  insert into public.item_variant_categories (item_variant_id, category_id)
  select v_variant_id, category_id from unnest(p_category_ids) as selected(category_id);
  return v_variant_id;
end;
$$;

revoke all on function public.save_catalog_variant(
  uuid, uuid, text, text, text, text, text, text, text, uuid, uuid[], boolean, integer, boolean
) from public;
revoke all on function public.save_catalog_variant(
  uuid, uuid, text, text, text, text, text, text, text, uuid, uuid[], boolean, integer, boolean
) from anon;
